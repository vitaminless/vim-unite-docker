let s:kind = {
	...
	\ 'action_table': {
		...
		\ 'run':     { 'is_selectable': 1, 'description': 'run this image', },
		\ 'remove':  { 'is_selectable': 1, 'description': 'remove this image', },
	\ },
	\ 'parents': [],
\ }
...
function! s:kind.action_table.run.func(candidates)
	if len(a:candidates) != 1
		echo "candidates must be only one"
		return
	endif

	call s:docker_cmd('docker run ' . a:candidates[0].action_word)
endfunction
...
function! s:kind.action_table.remove.func(candidates)
	...
	call s:docker_cmd('docker rmi ' . a:candidates[0].action_word)
endfunction
...
function! unite#kinds#docker_images#define()
	return s:kind
endfunction
